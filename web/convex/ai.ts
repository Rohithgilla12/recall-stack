...
export const generateSummaryAndTags = internalAction({
	...
	handler: async (ctx, { content }) => {
		...
		try {
			...
			// 1. Generate Summary
			try {
				const summaryResult = await contentAgent.generateText(
					ctx,
					{},
					{
			...
			// 2. Generate Tags
			try {
				const tagsResult = await contentAgent.generateObject(
					ctx,
					{},
					{
						...
						schema: z.object({
							tags: z
								.array(z.string())
								.describe("An array of 3 to 5 relevant tags."),
						}),
					},
				)
				...
				if (error) error += `; Error generating tags: ${e}`
				else error = `Error generating tags: ${e}`
			}
