  ...
  handler: async (ctx, { content }) => {
    ...
    try {
      ...
      const combinedSchema = z.object({
        summary: z.string().describe("A concise summary of the content (2-3 sentences). Focus on main topics and key takeaways. If summary cannot be generated (e.g. content too short/nonsensical), return a brief explanation like 'Content not suitable for summarization'."),
        tags: z.array(z.string()).describe("An array of 3 to 5 relevant tags (single words or short phrases). If no relevant tags can be generated, return an empty array."),
      });
...
- For tags: return an empty array.`;
      ...
      try {
        const result = await contentAgent.generateObject(ctx, {}, {
          prompt: prompt,
        ...
        if (summary && summary.toLowerCase().includes("not suitable for summarization")) {
            console.log(`Content marked as not suitable for summary by LLM for content starting with: ${content.substring(0,50)}...`);
        }
        if (tags.length === 0) {
            console.log(`LLM returned no tags, potentially due to content suitability for content starting with: ${content.substring(0,50)}...`);
        }

      } catch (e: any) {
        console.error("Error generating summary and tags in a single call:", e);
        error = `Error in combined AI call: ${e.message}`;
      ...
      return { summary: "Unhandled AI error.", tags: [], error: `Unhandled error: ${e.message}` };
    }
  },
});
